...
export default function ViewEntriesPage({ user }: ViewEntriesPageProps) {
  ...
  const filteredEntries = allEntries.filter((entry) => {
    ...
    const matchesDistrict = !filterDistrict || entry.district === filterDistrict
    ...
    <div className="space-y-6">
      ...
      <Card className="p-4 sm:p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div>
            <label className="text-sm font-medium text-foreground block mb-2">Search by Tag or Owner</label>
          ...
          <div>
            ...
            <Select value={filterDistrict} onValueChange={setFilterDistrict}>
              ...
              <SelectContent>
                <SelectItem value={user.district}>{user.district}</SelectItem>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <table className="w-full">
                  <thead className="bg-secondary">
                    <tr>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-foreground">Tag Number</th>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-foreground">Owner</th>
                      <th className="px-6 py-3 text-left text-sm font-semibold text-foreground">Animal Type</th>
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredEntries.map((entry) => (
                      <tr key={entry.id} className="hover:bg-secondary/50 transition">
                        <td className="px-6 py-4 text-sm font-medium text-foreground">{entry.tagNumber}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{entry.ownerName}</td>
                        <td className="px-6 py-4 text-sm text-muted-foreground">{entry.animalType}</td>
              ...
              {filteredEntries.map((entry) => (
                <Card key={entry.id} className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <p className="font-semibold text-foreground">{entry.tagNumber}</p>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-xs">
                    <div>
                      ...
                      <p className="font-medium text-foreground">{entry.animalType}</p>
                    </div>
                    <div>
                      <p className="text-muted-foreground">Date</p>
                      <p className="font-medium text-foreground">{new Date(entry.date).toLocaleDateString("en-NP")}</p>
                    </div>
                  </div>
                </Card>
              ))}
      ...
      <Card className="p-4 sm:p-6 bg-secondary/50">
        <p className="text-sm text-muted-foreground">
          ...
          <span className="font-semibold text-foreground">{allEntries.length}</span> entries
        </p>
      </Card>
    </div>
  )
